...
if not CONTAINER_HOSTS:
    ...
    logger.error("Please set ANSIBLE_INVENTORY_PATH or DOCKER_/PODMAN_*_ENDPOINT environment variables")


async def container_api_request(host: str, endpoint: str, timeout: int = 5) -> Optional[Dict]:
    """
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(url, timeout=aiohttp.ClientTimeout(total=timeout)) as response:
                if response.status == 200:
                    ...
                    logger.warning(f"{runtime.capitalize()} API returned HTTP {response.status} for {host} ({url})")
                    return None
    except asyncio.TimeoutError:
        logger.error(f"Timeout connecting to {runtime} API on {host} (timeout={timeout}s)")
        return None
...
def normalize_container_info(container: Dict, runtime: str) -> Dict:
    ...
    if runtime == 'podman':
        ...
        return {
            'Id': container.get('Id', ''),
            'Names': [container.get('Names', ['Unknown'])[0] if isinstance(container.get('Names'), list) 
...
async def handle_call_tool(
    ...
    try:
        if name == "get_docker_containers":
            default_hostname = list(CONTAINER_HOSTS.keys())[0] if CONTAINER_HOSTS else "no-hosts-configured"
            hostname = arguments.get("hostname", default_hostname) if arguments else default_hostname
                    ...
                    if ports:
                        ...
                        for port in ports:
                            if 'PublicPort' in port:
                                port_mappings.append(f"{port.get('PublicPort', '?')}->{port.get('PrivatePort', '?')}")
                        if port_mappings:
            ...
            for container in containers[:10]:  # Limit to 10 for performance
                ...
                stats = await container_api_request(hostname, stats_endpoint_template.format(container_id), timeout=10)
                ...
                if stats:
                    ...
                    try:
                        ...
                        mem_usage = mem_stats.get('usage', 0) / (1024**3)  # Convert to GB
