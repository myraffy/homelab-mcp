...
class AnsibleInventoryMCP:
    ...
    def _get_host_details(self, hostname: str) -> dict:
        ...
        return {
            "hostname": hostname,
            "details": all_hosts['hosts'][hostname]
        }
            ...
            if isinstance(data, dict):
                ...
                for key, value in data.items():
                    ...
                    if isinstance(value, dict):
                        result = find_group(value, target, f"{path}/{key}" if path else key)
                        if result:
                            return result
            return None
    ...
    def _search_hosts(self, pattern: Optional[str] = None, 
        ...
        return {
            "total_matches": len(matching_hosts),
