...
async def fetch_unifi_data():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if not json_files:
            raise FileNotFoundError(f"No output file generated. STDOUT: {stdout}, STDERR: {stderr}")
...
def format_network_clients(data: dict) -> list[types.TextContent]:
    ...
    for network_id, network_clients in sorted(by_network.items(), key=lambda x: len(x[1]), reverse=True):
...
def format_network_summary(data: dict) -> list[types.TextContent]:
    ...
    for network_id, count in sorted(by_network.items(), key=lambda x: x[1], reverse=True)[:5]:
