...
class UnifiAPI:
    ...
    def login_with_api_key(self, api_key):
        ...
        # Test the connection
        try:
            response = self.session.get(f"{self.base_url}/proxy/network/api/s/{self.site}/self")
    ...
    def _get(self, endpoint):
        ...
        if response.status_code == 200:
            ...
            print(f"Warning: Failed to fetch {endpoint} (status: {response.status_code})", file=sys.stderr)
            return []
    ...
    try:
        ...
        for client in clients:
            client_info = {
                ...
                'connected_device': client.get('sw_mac', client.get('ap_mac', 'Unknown')),
...
Examples:
  ...
  %(prog)s --host 192.168.1.1 --api-key YOUR_KEY --port 8443 --site default
    ...
    auth_group.add_argument('--username', help='Controller username (requires --password)')
    
    parser.add_argument('--password', help='Controller password (used with --username)')
    parser.add_argument('--port', default=443, type=int, help='Controller port (default: 443)')
    parser.add_argument('--site', default='default', help='Site ID (default: default)')
    parser.add_argument('--output-dir', default='./unifi_export', help='Output directory')
    ...
    print(f"Connecting to Unifi Controller at {args.host}:{args.port}...", file=sys.stderr)
    ...
    if not success:
        ...
        print("  - Verify the controller is accessible at https://{}:{}".format(args.host, args.port), file=sys.stderr)
    ...
    print(f"  - Port Forwarding Rules: {len(network_info['port_forwarding'])}", file=sys.stderr)
